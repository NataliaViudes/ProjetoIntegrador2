...
public class DAOVincularBeneficiario {
    ...
    public List<Beneficiario> buscarBeneficiariosPorAgendamento(int idAgendamento) {
        List<Beneficiario> lista = new ArrayList<>();

        String sql = """
        SELECT b.*
        FROM beneficiario b
        JOIN vincularbeneficiario vb
            ON b.id = vb.idBeneficiario
        WHERE vb.idAgendamento = ?
        ORDER BY b.nome
    """;
        ...
        try (PreparedStatement stmt = bd.preparar(sql)) {
            ...
            System.out.println("Erro ao buscar beneficiários vinculados: " + e.getMessage());
        }
